...
const getKey = (pageIndex: number, previousPageData: SongResponse | null, searchParams: SongSearchParams) => {
  ...
  if (searchParams.sortBy && searchParams.sortBy !== "releaseDate") params.set("sortBy", searchParams.sortBy)
  if (searchParams.sortOrder && searchParams.sortOrder !== "desc") params.set("sortOrder", searchParams.sortOrder)
  if (searchParams.type && searchParams.type !== "all") params.set("type", searchParams.type)

  return `/api/songs/search?${params.toString()}`
}

export function useInfiniteSongs(searchParams: SongSearchParams) {
  const { data, error, size, setSize, isValidating, isLoading, mutate } = useSWRInfinite<SongResponse>(
    (pageIndex, previousPageData) => getKey(pageIndex, previousPageData, searchParams),
    fetcher,
    {
